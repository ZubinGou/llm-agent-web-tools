...
def get_engines():
    ...
    base_dir = os.getcwd()
    engines_dir = os.path.join(base_dir, 'web_tools', 'core', 'engines')
    ...
    for filename in os.listdir(engines_dir):
        if os.path.isfile(os.path.join(engines_dir, filename)) and filename.endswith('.py') \
            ...
            module = import_module("web_tools.core.engines.{}".format(engine.lower()))
...
class EngineBaseTest(unittest.TestCase):
    ...
    def setUp(self):
        from web_tools.core.engines.google import Search # pylint: disable=import-outside-toplevel
        self.engine = Search()

    @patch('web_tools.core.engines.google.Search.get_results')
    @patch('web_tools.core.engines.google.Search.get_soup')
    ...
    def test_two_queries_different_results(self):
        """ Test that url updates work fine """
        from web_tools.core.engines.google import Search as GoogleSearch # pylint: disable=import-outside-toplevel
        from web_tools.core.engines.yahoo import Search as YahooSearch # pylint: disable=import-outside-toplevel
...
class TestScraping(unittest.TestCase):
    ...
    provides tests for title, description and return urls
    """
    engine_class = None
    ...
    def test_returned_results(self):
        ...
        """
        self.assertTrue(len(self.results['title']) >= 4)
        self.assertTrue(len(self.results['link']) >= 4)
        # coursera does not return description for
        # Preaching to the choir
        if not self.engine.name.lower() == "coursera":
            self.assertTrue(len(self.results['description']) >= 4)
        else:
    ...
    def test_links(self):
        for link in self.results['link']:
            ...
            # Sometimes googlescholar returns empty link for citation type results
    ...
    def test_results_length_are_the_same(self):
        ...
        # So if all keys are not the same length check that the title and link length are
        # the same
        default_keys = ["title", "link"]
